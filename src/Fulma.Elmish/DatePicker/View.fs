...
module View =
    ...
    let internal viewWithRendererInternal<'Msg> render =
        FunctionComponent.Of(fun (props: {| Config: Config<'Msg>; State: State; CurrentDate: DateTime option; Dispatch: 'Msg -> unit |}) ->
            ...
            let elements =
                { Input = input config state.current state.update currentDate
                  DeleteButton =
    ...
    let renderAsFieldBody elements =
        div [ ]
            [ yield Field.body []
                [ Field.div (if elements.DeleteButton.IsSome then [Field.HasAddons] else [])
                    [ yield Control.p [ Control.IsExpanded ] [ elements.Input ]
                      if elements.DeleteButton.IsSome then
                          yield Control.p [] [ elements.DeleteButton.Value ] ] ]
              if elements.Calendar.IsSome then
                yield elements.Calendar.Value ]

    let viewWithRenderer render config state currentDate dispatch =
        viewWithRendererInternal render
            {| Config = config
               State = state
    ...
    let root config state currentDate dispatch =
        viewWithRendererInternal renderAsFieldBody
            {| Config = config
               State = state
               CurrentDate = currentDate
               Dispatch = dispatch |}
