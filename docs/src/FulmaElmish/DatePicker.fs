...
If you need a different layout rendered than `DatePicker.View.root` provides, try `viewWithRenderer`:

```fsharp
let renderDatePicker (elements : DatePicker.Types.Elements) =
    div [ ]
        [ yield Field.body []
...
let root model dispatch =
    DatePicker.View.viewWithRenderer renderDatePicker pickerConfig initialPickerState model.CurrentDate dispatch
```
